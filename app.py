...
def get_posts():
    ...
    try:
        ...
        for post in posts:
            ...
            post["count_thumbs"] = db.thumbss.count_documents(
                {"post_id": post["_id"], "type": "thumbs-up"}
            )
            post["thumbs_by_me"] = bool(
                db.thumbss.find_one(
                    {"post_id": post["_id"], "type": "thumbs-up", "username": payload["id"]}
...
def update_thumbs():
    ...
    try:
        ...
        if action_receive == "thumbs-up":
            db.thumbss.insert_one(doc)
        else:
            db.thumbss.delete_one(doc)
        count = db.thumbss.count_documents(
            {"post_id": post_id_receive, "type": type_receive}
        )
